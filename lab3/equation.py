        ...
        for ind in range(point_num):
                if (calc_equation(expression, root_range[0] + (ind) * step) 
                    * calc_equation(expression, root_range[0] + (ind + 1) * step) < 0):
                    ...
                    graph_insert_diff(root_range[0] + (ind + 1/2)* step, root_range[1] - (root_range[0] + (ind + 1/2) * step), (ind + 1/2) * step)
        if out_key:
             break
        else:
             point_num += 2
...
def iteration_method():
    ...
    while abs(calc_equation(expression, approx)) > sigma: 
        ...
        graph_insert(approx, math.sqrt(abs(calc_equation(expression, approx))), x_iter, error_iter)
...
def newton_method():
     ...
     x_0 = root_range[0] #(root_range[1] + root_range[0]) / 2
     approx = x_0

     while abs(calc_equation(expression, approx)) > sigma: 
        approx = approx - calc_equation(expression, approx) / calc_equation(str(diff(my_func, my_symbols['x'])), x_0)
        # print(approx)
        graph_insert(approx, math.sqrt(abs(calc_equation(expression, approx))), x_newton, error_newton)
...
string = str(diff(my_func, my_symbols['x']))
print(eval(string.replace('x', '12')))
"""
...
plt.errorbar([ind + 1 for ind in range(len(x_list))], x_list, yerr=error_list, fmt='o', ecolor='g', linewidth=1)
...
plt.legend(['Dividing a segment in half', 'Newton method'], loc='upper left', fontsize='7')
